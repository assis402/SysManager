    ...
    public class ProductPutRequestValidator : AbstractValidator<ProductPutRequest>
    {
                                          ...
                                          )
        {
            ...
            RuleFor(product => product)
                .Must(product =>
                {
                 var exists = repository.GetByProductCodeAsync(product.ProductCode).Result;
            ...
            RuleFor(product => product.ProductTypeId)
                            .Must(productType =>
                            {
                                try
                                {
                                    var exists = productTypeRepository.GetByIdAsync(Guid.Parse(productType)).Result;
                                    return exists != null;
                                }
                                catch (Exception)
                                {
            ...
            RuleFor(product => product.CategoryId)
                            .Must(category =>
                            {
                                try
                                {
                                    var exists = categoryRepository.GetByIdAsync(Guid.Parse(category)).Result;
                                    return exists != null;
                                }
                                catch (Exception)
                                {
